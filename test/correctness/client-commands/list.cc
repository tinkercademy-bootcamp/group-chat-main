...
// Test list without any channels without space
TEST_F(CommandListTest, TestListNoChannelNoSpace) {
...
// Test list without any channels with space
TEST_F(CommandListTest, TestListNoChannelWithSpaces) {
  client1_->send_message("/list       ");
  EXPECT_TRUE(check_last_log_level(spdlog::level::info)) << "Expected INFO level for successful list";
  EXPECT_TRUE(wait_for_response_from_client(client1_.get(), {"Channels:"}));
}

// Test list for a single channel
TEST_F(CommandListTest, TestListSingleChannel) {
  client1_->send_message("/create Test Channel 1");
  EXPECT_TRUE(check_last_log_level(spdlog::level::info)) << "Expected INFO level for successful create";
  EXPECT_TRUE(wait_for_response_from_client(client1_.get(), {"Channel created."}));

  client1_->send_message("/list");
  EXPECT_TRUE(check_last_log_level(spdlog::level::info)) << "Expected INFO level for successful list";
  EXPECT_TRUE(wait_for_response_from_client(client1_.get(), {"Test Channel 1"}));
}

// Test list for multiple channels
TEST_F(CommandListTest, TestListMultipleChannels) {
  client1_->send_message("/create Test Channel 1");
  EXPECT_TRUE(check_last_log_level(spdlog::level::info)) << "Expected INFO level for successful create";
  EXPECT_TRUE(wait_for_response_from_client(client1_.get(), {"Channel created."}));
  ...
  client1_->send_message("/list");
  EXPECT_TRUE(check_last_log_level(spdlog::level::info)) << "Expected INFO level for successful list";
  EXPECT_TRUE(wait_for_response_from_client(client1_.get(), {"Test Channel 1", "TestChannel2"}));
}

// Test list for large channel
TEST_F(CommandListTest, TestListSingleLargeChannel) {
  ...
  client1_->send_message("/list");
  EXPECT_TRUE(check_last_log_level(spdlog::level::info)) << "Expected INFO level for successful list";
  EXPECT_TRUE(wait_for_response_from_client(client1_.get(), {channel_name}));
}


// Test list for large return distributed over multiple channels
TEST_F(CommandListTest, TestListLargeReturn) {
  std::string channel_name(2050, 'a');
  client1_->send_message("/create " + channel_name);
  EXPECT_TRUE(check_last_log_level(spdlog::level::info)) << "Expected INFO level for successful create";
  EXPECT_TRUE(wait_for_response_from_client(client1_.get(), {"Channel created."}));
  ...
  client1_->send_message("/list");
  EXPECT_TRUE(check_last_log_level(spdlog::level::info)) << "Expected INFO level for successful list";
  EXPECT_TRUE(wait_for_response_from_client(client1_.get(), {channel_name, "TestChannel2"}));
}

// Test list for multiple clients
TEST_F(CommandListTest, TestListMultipleClients) {
  client1_->send_message("/create Test Channel 1");
  EXPECT_TRUE(check_last_log_level(spdlog::level::info)) << "Expected INFO level for successful create";
  EXPECT_TRUE(wait_for_response_from_client(client1_.get(), {"Channel created."}));

  client2_->send_message("/list");
  EXPECT_TRUE(check_last_log_level(spdlog::level::info)) << "Expected INFO level for successful list";
  EXPECT_TRUE(wait_for_response_from_client(client2_.get(), {"Test Channel 1"}));
}
