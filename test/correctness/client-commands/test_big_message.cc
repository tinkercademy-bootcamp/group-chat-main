...
// Test fixture for big message send tests
class BigSendTest : public ::testing::TestWithParam<int> {
protected:
  void SetUp() override {
    server_thread_ = std::thread([this]() {
      server_ = std::make_unique<server::EpollServer>(test_port_);
    ...
    for(int i=0; i<CLIENT_COUNT; i++) {
      clients.emplace_back(std::make_unique<client::Client>(test_port_, "127.0.0.1"));
      std::string name_cmd = "/name user" + std::to_string(i);
      clients.back()->send_message(name_cmd);
    }
    std::this_thread::sleep_for(std::chrono::milliseconds(300));
    for(int i=0; i<CLIENT_COUNT; i++) {
...
TEST_P(BigSendTest, CheckSendReliability) {
  int msg_size = GetParam();
  std::string big_msg(msg_size, 'A');
  clients[0]->send_message("/create BigMsgTest");
  EXPECT_TRUE(wait_for_response(0, "Channel created"));
  std::this_thread::sleep_for(std::chrono::milliseconds(100));
  for(int i=0; i<CLIENT_COUNT; i++) {
    clients[i]->send_message("/join BigMsgTest");
  }
  std::this_thread::sleep_for(std::chrono::milliseconds(100));
  // Only client 0 sends the big message
  clients[0]->send_message("/message " + big_msg);
  std::this_thread::sleep_for(std::chrono::milliseconds(1000));
  // Check that each client (except sender) received the message
  for(int i=1; i<CLIENT_COUNT; i++) {
    auto str_vec = receive_messages_from_client(clients[i].get(), 2000);
    std::string final = "";
    for(auto& s: str_vec) final += s;
    bool found = final.find(big_msg) != std::string::npos;
    EXPECT_TRUE(found) << "Client " << i << " did not receive the big message of size " << msg_size;
  }
}
...
INSTANTIATE_TEST_SUITE_P(
    MessageSizes,
    BigSendTest,
    ::testing::Values(1, 10, 100, 1000, 2000, 5000, 10000, 15000, 20000)
);
